...
function FindNodeChild(message_id, node_arr){

}
function ConstructMessageHistoryTree(parent_node_id, messages_array, bucket){
    
    for(var i =0 ;i < messages_array.length; ++i)
    {
        current_msg = messages_array[i];
        if(current_msg.FROM_PARENT_ID == parent_node_id)
        {
            bucket.push(current_msg)
            
        }
    }
}

function ConstructMessageHistoryTree(messages_array, current_parent_id, bucket){
    ...
    while(i < messages_array.length)
    {
        ...
        {   
            ...
            bucket.push(current_msg_json);
            messages_array.splice(i, 1);
            i = i -1;
            if(i < 0) i = 0;

            bucket_children = ConstructMessageHistoryTree(messages_array, current_msg_json.MESSAGE_ID, []);
            if(bucket_children.length != 0){
                bucket.push(bucket_children)
            }
            continue;
        }
        ++i;
...
function Driver(messages_array)
{   
    tree = ConstructMessageHistoryTree(messages_array, "", []);
    return tree;
}
...
function ClientAcknowledgeMessageHistoryCache(packet){
    message_tree = ConstructMessageHistoryTree(packet.MESSAGE_HISTORY, "", []);
    for(var i = 0; i < message_tree.length; ++i){
        AppendMessage(message_tree[i])
    }
    alert("done");
    // for(var i = 0; i < message_tree.length; ++i){
    //     AppendMessage(message_tree[i])
